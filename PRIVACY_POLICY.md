...
>
> 本政策是草拟稿，文中所有「【】」占位内容需由运营方在发布前填写并核对。
...
5. **注销账号**：您可以通过【填写方式，例如：联系客服提出注销申请】申请注销账号。注销后，我们将删除或匿名化处理您的账号信息及相关数据（包括作品、互动、预约、会员、聊天等关联数据），法律法规要求保留的除外。【说明：当前应用内暂未提供自助注销入口，建议在"设置"中补充自助注销功能以满足应用商店审核要求】；
...
- [ ] 填写生效日期、运营主体名称与注册地址、客服邮箱/电话；
- [ ] 确认服务器与数据存储地区，并填入"存储地点"；
- [ ] 确认生产环境已启用 HTTPS/TLS；
- [ ] 确认未成年人准入年龄与监护人同意规则；
- [ ] 确认账号注销受理方式；如已上线自助注销入口，请补充对应路径；
- [ ] 确认应用内"意见反馈/投诉"入口已上线，并填写对应路径；
- [ ] 若后续接入微信/支付宝等线上支付渠道、推送 SDK、统计分析 SDK，需同步更新本政策。

本文档依据仓库代码（`server/src` 用户/预约/会员/社区/视频/聊天/通知/上传/审核等模块，`app/` 客户端权限与本地存储）整理，发布前请由运营与法务复核。
