...
class _MyCheckInQrPageState extends State<MyCheckInQrPage> {
  ...
  void _showQrDetail(Appointment appt) {
    ...
    showCheckInQrDialog(context, appt).whenComplete(() {
      if (mounted) setState(() => _qrDialogOpen = false);
    });
  }

  /// 取消待核销预约（与「我的订单」同一接口与确认文案）
  Future<void> _cancel(Appointment appt) async {
    ...
    try {
      await ApiClient.instance.post('/appointments/${appt.id}/cancel');
      if (!mounted) return;
      _load();
    } on DioException catch (e) {
      if (!mounted) return;
                            ...
                            child: Container(
                              ...
                              child: Column(
                                ...
                                children: [
                                  Row(
                                    children: [
                                      CheckInQrCode(code: appt.code, size: 96),
