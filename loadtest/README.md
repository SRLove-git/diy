...
上线前压测 Think Origin 服务端，聚焦**预约 / 核销（含上钟下钟）/ 会员**三大核心链路。套件包含：
...
可选：管理员账号（压管理端预约列表，只读）：

```bash
cat > loadtest/admin-accounts.json <<'EOF'
["admin:admin123456"]
EOF
```
...
BASE_URL=http://<服务器IP>:3000 PROFILE=load VUS=50 HOLD_MIN=5 \
  ACCOUNTS_FILE=/path/to/loadtest/accounts.json \
  ADMIN_ACCOUNTS_FILE=/path/to/loadtest/admin-accounts.json \
  k6 run loadtest/k6/load.js
...
| 流程 | 权重 | 覆盖接口 |
| --- | --- | --- |
| 预约 | 40% | 门店列表、门店详情（选桌位）、活动列表、桌位可用性、活动场次、创建预约（40% 概率）、我的预约、预约详情 |
| 核销 | 25% | 按预约码查询（公开）、输码核销（核销即上钟）、下钟（60% 概率） |
| 会员 | 30% | 套餐列表、我的会员、会员订单、我的卡券、钱包、会员经验、提交开通申请（5% 概率） |
| 其他 | 5% | 健康检查、管理端预约列表（配置了管理员账号时） |

**数据说明**：创建预约、核销、下钟、会员下单都会真实写库，压测会在测试库留下对应数据；预约的核销/下钟操作有状态流转，同一预约码只能核销一次，脚本按虚拟用户各自维护「创建 → 核销 → 下钟」的完整生命周期（每个 VU 同时只保留一个待核销预约，串行走完再开新单）。并发抢同一时段/桌位返回 4xx 冲突属于**业务正常行为**，脚本的错误率阈值只统计 5xx，4xx 请通过 checks 和状态码分布观察。
...
| `PROFILE` | 场景 | `load` |
| `ACCOUNTS_FILE` | 用户测试账号 | 无 |
| `ADMIN_ACCOUNTS_FILE` | 管理员账号（可选） | 无 |
